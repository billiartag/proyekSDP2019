...
public class fragment_flashsale_search extends Fragment {
    ArrayList<barang> kumpulanbarang = new ArrayList<barang>();
    RecyclerView rv_flashsale_search;
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View myview=inflater.inflate(R.layout.fragment_flashsale_search_layout,container,false);
        rv_flashsale_search=myview.findViewById(R.id.rv_flashsale_search);
        load_data_barang();
        return  myview;
    }
        ...
        FirebaseDatabase.getInstance().getReference().child("BarangDatabase").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot ds :dataSnapshot.getChildren()) {
                    if (!FirebaseAuth.getInstance().getCurrentUser().getEmail().equals(ds.child("idpenjual").getValue().toString()) && ds.child("status").getValue().toString().equals("1") && ds.child("jenis").getValue().equals("Flash Sale")){
                ...
                rv_flashsale_search.setHasFixedSize(true);
                rv_flashsale_search.setLayoutManager(new GridLayoutManager(getContext(),2));
                SearchFeedAdapter adapter = new SearchFeedAdapter(getActivity(), kumpulanbarang,((search_fragment) getParentFragment()).spinner_negara_search , ((search_fragment) getParentFragment()).checkBox_negara_search, ((search_fragment) getParentFragment()).edekategori_search, ((search_fragment) getParentFragment()).checkBox_kategori_search, ((search_fragment) getParentFragment()).searchView);
                rv_flashsale_search.setAdapter(adapter);
